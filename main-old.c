...
int main(int argc, char** argv)
{
	...
	long fsize, linenum = 0, linenum_alloc, linepos = 0, linepos_alloc, lineoffset = 0, lineoffset_alloc, fpos = 0, lines_per_thread;
	if(argc < 2)
	{
	...
	for(thread_cnt = 0; thread_cnt < NUM_THREADS; thread_cnt++)
	{
		...
		threadargs[thread_cnt].datalen = lineoffsets[(thread_cnt + 1) * lines_per_thread] - lineoffsets[thread_cnt * lines_per_thread];
